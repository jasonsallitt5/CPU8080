/*
useful videos to help:
https://www.youtube.com/watch?v=RkIHDG90ljc
https://www.youtube.com/watch?v=OpZ_soK--TQ

this is the instruction set
https://pastraiser.com/cpu/i8080/i8080_opcodes.html

guide to all the instructions
https://altairclone.com/downloads/manuals/8080%20Programmers%20Manual.pdf

8080 reference lists all the instructions
http://emulator101.com/

*/

//used to get the working directory
#ifdef WINDOWS
#include <direct.h>
...
int main(int argc, char** argv){
    ...
    /*
    std::cout << "Printing all argv..." << std::endl;
    if (argc !=0){
    ...
    */
    ...
    if(myFile.is_open()){
        ...
        //this will redirect output to a log.txt file in the same directory
        std::cout << "Starting Disassembler (from binary to readable assembly code)\n" 
                  << "Stdout is redirected to the log file \"asm_log.txt\" located in: " 
                  << get_current_dir() << std::endl;
        freopen("asm_log.txt","w",stdout);

        
        time_t now = time(0);   // current date/time based on current system
        char* dt = ctime(&now); // convert now to string form

        std::cout << "Log file created on: " << dt;




        //read the file into memory and find the size of the file

        myFile.seekg(0, std::ios::end); //sets postition of the stream (to the end)
        //tellg returns the current "get" position of the pointer the stream
        int file_size = myFile.tellg(); 
        myFile.seekg(0, std::ios::beg); //reset it to the beginning
        ...
        //create and read it into the buffer
        ...
        //perform disassembly
        //increment the program counter until the end of the file is reached
        int pc = 0;

        //CHANGED FOR TEST SHOULD BE file_size
        while (pc < file_size){
        ...
        std::cout << "file didnt open, please provide the file you intend"
                     "to disassemble as a cmd line arg" << std::endl;
    }
...
/*
input:  buffer, the stream of data being inputted from the file
        pc, where we are in the buffer (program counter)
output: opbytes, returns the number of times the pc counter needs to be 
        incremented. for example jmp needs the next two bytes after for the 
        address. therefore its incremented 3 times.
*/
int disassemble(char* buffer, int pc){

    char* signedOpcode = &buffer[pc];

    /*
    We are casting it to an unsigned char because earlier when trying to read
    it only accepted the signed char (maybe because its c++ and not c?). 
    But for the switch statment to allow all the options 0-FF (256 options)
    we need it to be unsigned.
    */
    unsigned char* opcode = reinterpret_cast<unsigned char*>(signedOpcode);


    //the example I'm following uses a giant switch statement might change
    //when implementing the whole cpu 

    //printing hex 
    printf("0x%04X: ", pc);


    //this tracks how many bytes we need to read which can vary per instruction
    //this also tells us how we increment the pc at the end
    int opbytes = 1;


    // Question: why do we put the 2nd byte as the first portoin of the address? 
    // (it was like this in the example code)

    //note: "-" are not commands and shouldn't be called
    ...
    //pc is incrememented based on opbytes
    return opbytes;
}
